...
def main():
    ...
    # Получаем компании через модуль Gis
    companies = Gis.get_companies(city, company_name)

    for company in companies:
        print(f"Обрабатываем компанию: {company['name']}")
        url = company.get('site_url', None)
        if not url:
            print(f"Компания {company['name']} не имеет сайта.")
            continue
